...
def directory_preprocessing():
    images_path = Path('../data/manifest-1608266677008')
    ...
    StudyInstanceUID_reduced = remove_dup(StudyInstanceUID_items)
    info = []
    overall_dir = '../data/images'
        ...
        for j in range(0,len(SeriesInstanceUID_reduced)):
            ...
            for b in range(0,len(df)):
                if SeriesInstanceUID_reduced[j] == df.iloc[b].SeriesInstanceUID:
                    copy2(df.iloc[b].fn,'../data/images/{}/{}/{}.dcm'.format(StudyInstanceUID_reduced[i],SeriesInstanceUID_reduced[j],df.iloc[b].SOPInstanceUID))
